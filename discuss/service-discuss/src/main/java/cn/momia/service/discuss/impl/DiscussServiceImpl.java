...
public class DiscussServiceImpl extends AbstractService implements DiscussService {
    ...
    private List<DiscussTopic> listTopics(Collection<Integer> topicIds) {
        String sql = String.format("SELECT A.Id, A.CityId, A.Cover, A.Title, A.Content, COUNT(DISTINCT B.UserId) AS Joined FROM SG_DiscussTopic A LEFT JOIN SG_DiscussReply B ON A.Id=B.TopicId AND B.Status=1 WHERE A.Id IN (%s) AND A.Status=1 GROUP BY A.Id", StringUtils.join(topicIds, ","));
