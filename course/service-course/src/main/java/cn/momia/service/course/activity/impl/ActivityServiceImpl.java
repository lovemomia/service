...
public class ActivityServiceImpl extends AbstractService implements ActivityService {
    ...
    public Activity getActivity(int activityId) {
        String sql = "SELECT Id, Icon, Cover, Title, `Desc`, Message, NeedPay, Price, StartTime, EndTime, ForNewUser, Detail, Stock AS TotalStock, UnlockedStock FROM SG_Activity WHERE Id=? AND OnlineTime<=NOW() AND OfflineTime>NOW() AND Status=1";
    ...
    public boolean isNewUser(String mobile) {
        String sql = "SELECT COUNT(1) FROM SG_ActivityEntry WHERE Mobile=? AND Status<>0";
