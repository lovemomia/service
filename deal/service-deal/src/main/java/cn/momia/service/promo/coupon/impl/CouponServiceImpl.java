...
public class CouponServiceImpl extends DbAccessService implements CouponService {
    ...
    public List<UserCoupon> queryByUser(long userId, long orderId, BigDecimal totalFee, int status, int start, int count) {
        ...
        if (status == UserCoupon.Status.EXPIRED) {
            String sql = "SELECT " + joinUserCouponFields() + " FROM t_user_coupon WHERE userId=? AND status=? AND endTime<=NOW() ORDER BY endTime ASC, startTime ASC, addTime DESC LIMIT ?,?";
            ...
            String sql = "SELECT " + joinUserCouponFields() + " FROM t_user_coupon WHERE userId=? AND consumption<=? AND ((status=? AND endTime>NOW()) OR (orderId=? AND status=?)) ORDER BY endTime ASC, startTime ASC, addTime DESC LIMIT ?,?";
            ...
            String sql = "SELECT " + joinUserCouponFields() + " FROM t_user_coupon WHERE userId=? AND status=? ORDER BY endTime ASC, startTime ASC, addTime DESC LIMIT ?,?";
            jdbcTemplate.query(sql, new Object[] { userId, status, start, count }, new RowCallbackHandler() {
                @Override
                public void processRow(ResultSet rs) throws SQLException {
                    UserCoupon userCoupon = buildUserCoupon(rs);
            ...
            String sql = "SELECT " + joinUserCouponFields() + " FROM t_user_coupon WHERE userId=? AND status<>0 ORDER BY endTime ASC, startTime ASC, addTime DESC LIMIT ?,?";
