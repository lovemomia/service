            ...
            <div class="box span12">
                <div class="row-fluid sortable">
                    <div class="box-header well" data-original-title>
                        <h2><i class="icon-user"></i> 分类管理</h2>
                        <div class="box-icon">
                            <a href="${ctx}/category/oper.do?uid=${user.id}&id=0&&pageNo=${queryPage.pageNo}" class="btn btn-add btn-round"><i class=" icon-plus"></i></a>
                        </div>
                    </div>
                        ...
                        <form >
                            <table class="table table-striped table-bordered bootstrap-datatable datatable">
                                <c:forEach items="${queryPage.list}" var="entity">
                                    <tr>
                                        <td>分类编号:<c:out value="${entity.id}"></c:out><br>
                                            父级分类:<c:out value="${entity.parentname}"></c:out><br>
                                            分类名称:<c:out value="${entity.name}"></c:out><br>
                                            添加时间:<c:out value="${fn:substring(entity.addTime,0,19)}"></c:out><br>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="center">
                                            <a class="btn btn-info" href="${ctx}/category/oper.do?uid=${user.id}&id=${entity.id}&pageNo=${queryPage.pageNo}">
                                                <i class="icon-edit icon-white"></i>
                                                修改
                                            </a>
                                            <a class="btn btn-danger" href="${ctx}/category/del.do?uid=${user.id}&id=${entity.id}&pageNo=${queryPage.pageNo}">
                                ...
                                <tr>
                                    <td class="center">
                                        ...
                                        <a href="${ctx}/category/info.do?uid=${user.id}&pageNo=${queryPage.topPageNo}">
                                            首页
                                        </a>
                                        <a href="${ctx}/category/info.do?uid=${user.id}&pageNo=${queryPage.previousPageNo}">
                                            上一页
                                        </a>
                                        <a href="${ctx}/category/info.do?uid=${user.id}&pageNo=${queryPage.nextPageNo}">
                                            下一页
                                        </a>
                                        <a href="${ctx}/category/info.do?uid=${user.id}&pageNo=${queryPage.bottomPageNo}">
                                            尾页
                                        </a>
