            ...
            <div class="box span12">
                <div class="row-fluid sortable">
                    <div class="box-header well" data-original-title>
                        <h2><i class="icon-user"></i> 属性管理</h2>
                        <div class="box-icon">
                            <a href="${ctx}/property/oper.do?uid=${user.id}&id=0&mark=0" class="btn btn-add btn-round"><i class=" icon-plus"></i></a>
                        </div>
                    </div>
                        ...
                        <form >
                            <table class="table table-striped table-bordered bootstrap-datatable datatable">
                                <c:forEach items="${entitys}" var="entity">
                                    <tr>
                                        <td>属性编号:<c:out value="${entity.id}"></c:out><br>
                                            属性名称:<c:out value="${entity.name}"></c:out><br>
                                            所属分类:<c:out value="${entity.categoryId}"></c:out><br>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="center">
                                            <a class="btn btn-success" href="${ctx}/property/oper.do?uid=${user.id}&id=${entity.id}&mark=1">
                                                <i class="icon-zoom-in icon-white"></i>
                                                属性值
                                            </a>
                                            <a class="btn btn-info" href="${ctx}/property/oper.do?uid=${user.id}&id=${entity.id}&mark=2">
                                                <i class="icon-edit icon-white"></i>
                                                修改
                                            </a>
                                            <a class="btn btn-danger" href="${ctx}/property/del.do?uid=${user.id}&id=${entity.id}">
