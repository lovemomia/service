            ...
            <div class="box span12">
                <div class="row-fluid sortable">
                    <div class="box-header well" data-original-title>
                        <h2><i class="icon-user"></i> 地址管理</h2>
                        <div class="box-icon">
                            <a href="${ctx}/place/oper.do?uid=${user.id}&id=0&mark=0&pageNo=${queryPage.pageNo}" class="btn btn-add btn-round"><i class=" icon-plus"></i></a>
                        </div>
                    </div>
                        ...
                        <form >
                            <table class="table table-striped table-bordered bootstrap-datatable datatable">
                                <c:forEach items="${queryPage.list}" var="entity">
                                    <tr>
                                        <td>地址编号:<c:out value="${entity.id}"></c:out><br>
                                            地址名称:<c:out value="${entity.name}"></c:out><br>
                                            地址详情:<c:out value="${entity.address}"></c:out><br>
                                            经度:<c:out value="${entity.lng}"></c:out><br>
                                            纬度:<c:out value="${entity.lat}"></c:out><br>
                                            描述:<c:out value="${entity.desc}"></c:out><br>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="center">
                                            <a class="btn btn-success" href="${ctx}/place/oper.do?uid=${user.id}&id=${entity.id}&mark=1&pageNo=${queryPage.pageNo}">
                                                <i class="icon-zoom-in icon-white"></i>
                                                图片
                                            </a>
                                            <a class="btn btn-info" href="${ctx}/place/oper.do?uid=${user.id}&id=${entity.id}&mark=2&pageNo=${queryPage.pageNo}">
                                                <i class="icon-edit icon-white"></i>
                                                修改
                                            </a>
                                            <a class="btn btn-danger" href="${ctx}/place/del.do?uid=${user.id}&id=${entity.id}&pageNo=${queryPage.pageNo}">
                                ...
                                <tr>
                                    <td class="center">
                                        ...
                                        <a href="${ctx}/place/info.do?uid=${user.id}&pageNo=${queryPage.topPageNo}">
                                            首页
                                        </a>
                                        <a href="${ctx}/place/info.do?uid=${user.id}&pageNo=${queryPage.previousPageNo}">
                                            上一页
                                        </a>
                                        <a href="${ctx}/place/info.do?uid=${user.id}&pageNo=${queryPage.nextPageNo}">
                                            下一页
                                        </a>
                                        <a href="${ctx}/place/info.do?uid=${user.id}&pageNo=${queryPage.bottomPageNo}">
                                            尾页
                                        </a>
