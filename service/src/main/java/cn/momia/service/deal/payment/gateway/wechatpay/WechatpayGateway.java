...
public class WechatpayGateway implements PaymentGateway {
    ...
    private void processResponseEntity(PrepayResult result, String entity, String tradeType) {
        ...
        if (successful) {
            ...
            if (tradeType.equals("NATIVE")) {
                result.add(WechatpayPrepayFields.PREPAY_RESULT_NATIVE_APPID, conf.getString("Payment.Wechat.NativeAppId"));
                result.add(WechatpayPrepayFields.PREPAY_RESULT_NATIVE_PARTNERID, conf.getString("Payment.Wechat.MchId"));
                result.add(WechatpayPrepayFields.PREPAY_RESULT_NATIVE_PREPAYID, params.get(WechatpayPrepayFields.PREPAY_ID));
                result.add(WechatpayPrepayFields.PREPAY_RESULT_NATIVE_PACKAGE, "Sign=WXPay");
                result.add(WechatpayPrepayFields.PREPAY_RESULT_NATIVE_NONCE_STR, WechatpayUtil.createNoncestr(32));
                result.add(WechatpayPrepayFields.PREPAY_RESULT_NATIVE_TIMESTAMP, String.valueOf(new Date().getTime()).substring(0, 10));
                result.add(WechatpayPrepayFields.PREPAY_RESULT_NATIVE_SIGN, WechatpayUtil.sign(result.getAll(), tradeType));
            } else if (tradeType.equals("JSAPI")) {
                ...
                result.add(WechatpayPrepayFields.PREPAY_RESULT_JSAPI_NONCE_STR, params.get(WechatpayPrepayFields.NONCE_STR));
