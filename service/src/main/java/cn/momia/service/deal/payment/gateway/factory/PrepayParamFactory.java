    ...
    public static PrepayParam create(Map<String, String[]> httpParams, Order order, int payType) {
        switch (payType) {
            ...
            case Payment.Type.WECHATPAY:
                return createWechatpayPrepayParam(httpParams, order);
            default:
                throw new RuntimeException("invalid pay type: " + payType);
        }
    }

    private static PrepayParam createWechatpayPrepayParam(Map<String, String[]> httpParams, Order order) {
        String spbill_create_ip = httpParams.get("spbill_create_ip")[0];
        String trade_type = httpParams.get("trade_type")[0];
        String openId = httpParams.get("openid")[0];
        String singStr = "";
        WechatpayPrepayParam payParam = new WechatpayPrepayParam();
